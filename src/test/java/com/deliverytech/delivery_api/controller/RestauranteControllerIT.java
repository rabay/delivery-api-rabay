...
class RestauranteControllerIT extends BaseIntegrationTest {
  ...
  void shouldGetRestauranteById_whenExists() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/restaurantes/{id}", saved.getId()).contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldGetRestauranteById_WithInvalidId_ReturnNotFound() throws Exception {
    mockMvc
        .perform(get("/api/restaurantes/{id}", 99999L).contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldUpdateRestaurante_andReturnUpdated() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/restaurantes/{id}", saved.getId())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void shouldInactivateRestaurante_andReturnOk() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/api/restaurantes/{id}", saved.getId()).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
  ...
  void shouldReturnPagedRestaurantes_andIncludeLinks() throws Exception {
    ...
    mockMvc
        .perform(get("/api/restaurantes?page=0&size=5").contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
