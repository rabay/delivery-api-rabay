...
class PedidoControllerIntegrationTest extends BaseIntegrationTest {
  ...
  private Long extractId(String json) {
    ...
    java.util.regex.Pattern pattern =
        java.util.regex.Pattern.compile("\"id\"\\s*:\\s*(\"?)(\\d+)\\1");
    ...
    if (matcher.find()) {
      ...
      if (idStart != -1) {
        ...
        while (idEnd < json.length()
            && (Character.isDigit(json.charAt(idEnd)) || json.charAt(idEnd) == '"')) {
  ...
  void criarPedido_e_buscarPorCliente_retornaListaComPedido() {
    ...
    ResponseEntity<String> authResp =
        restTemplate.postForEntity(
            "http://localhost:" + port + "/api/auth/login", authReq, String.class);
    ...
    String clienteJson =
        "{\"nome\": \"Cli Int\", \"email\": \"cli@test.com\", \"telefone\": \"11999999999\","
            + " \"endereco\": \"Rua Teste, 123\", \"senha\": \"123456\"}";
    HttpEntity<String> cliReq = new HttpEntity<>(clienteJson, headers);
    ResponseEntity<String> cliResp =
        restTemplate.postForEntity(
            "http://localhost:" + port + "/api/clientes", cliReq, String.class);
    ...
    System.out.println(
        "ClienteId class: " + (clienteId != null ? clienteId.getClass().getName() : "null"));
    assertThat(clienteId).isNotNull().isPositive();

    // Criar restaurante
    System.out.println("Creating restaurant...");
    String restauranteJson =
        "{\"nome\": \"Rest Int\", \"categoria\": \"Pizzaria\", \"endereco\": \"Rua Teste, 123\","
            + " \"taxaEntrega\": 5.0, \"tempoEntregaMinutos\": 30, \"telefone\": \"11999999999\","
            + " \"email\": \"rest@test.com\", \"avaliacao\": 4.0 }";
    HttpEntity<String> restReq = new HttpEntity<>(restauranteJson, headers);
    ResponseEntity<String> restResp =
        restTemplate.postForEntity(
            "http://localhost:" + port + "/api/restaurantes", restReq, String.class);
    ...
    String produtoJson =
        "{\"nome\": \"Prod Int\", \"categoria\": \"Pizza\", \"preco\": 9.9, \"disponivel\": true,"
            + " \"restauranteId\": "
            + restauranteId
            + ", \"quantidadeEstoque\": 10 }";
    HttpEntity<String> prodReq = new HttpEntity<>(produtoJson, headers);
    ResponseEntity<String> prodResp =
        restTemplate.postForEntity(
            "http://localhost:" + port + "/api/produtos", prodReq, String.class);
    ...
    String pedidoJson =
        "{\"clienteId\": "
            ...
            + ", \"restauranteId\": "
            + restauranteId
            + ", \"enderecoEntrega\": { \"rua\": \"Rua Teste\", \"numero\": \"123\", \"bairro\":"
            + " \"Bairro Teste\", \"cidade\": \"Cidade Teste\", \"estado\": \"SP\", \"cep\":"
            + " \"01000-000\" }, \"itens\": [{ \"produtoId\": "
            + produtoId
            + ", \"quantidade\": 1, \"precoUnitario\": 9.9 }] }";
    HttpEntity<String> pedidoReq = new HttpEntity<>(pedidoJson, headers);
    ResponseEntity<String> pedidoResp =
        restTemplate.postForEntity(
            "http://localhost:" + port + "/api/pedidos/dto", pedidoReq, String.class);
    ...
    System.out.println(
        "clienteId type: " + (clienteId != null ? clienteId.getClass().getName() : "null"));
