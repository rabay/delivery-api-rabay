...
class RestauranteControllerTest {
    ...
    void deveListarRestaurantesPaginados() throws Exception {
        List<RestauranteResponse> restaurantes = Arrays.asList(
            new RestauranteResponse(1L, "Rest 1", "Italiana", "Rua 1", BigDecimal.valueOf(5.0), "11999999999", "rest1@email.com", 30, BigDecimal.valueOf(4.5), true),
            new RestauranteResponse(2L, "Rest 2", "Chinesa", "Rua 2", BigDecimal.valueOf(6.0), "11888888888", "rest2@email.com", 25, BigDecimal.valueOf(4.2), true)
        );
    ...
    void deveAtualizarRestauranteComSucesso() throws Exception {
        ...
        when(restauranteService.atualizar(eq(1L), any(RestauranteRequest.class))).thenReturn(restauranteAtualizado);
    ...
    void deveBuscarRestaurantesPorCategoria() throws Exception {
        List<RestauranteResponse> restaurantes = Arrays.asList(
            new RestauranteResponse(1L, "Pizza Place", "Italiana", "Rua 1", BigDecimal.valueOf(5.0), "11999999999", "pizza@email.com", 30, BigDecimal.valueOf(4.5), true)
        );
    ...
    void deveAlterarStatusDoRestaurante() throws Exception {
        ...
        mockMvc.perform(patch("/api/restaurantes/1/status")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void deveCalcularTaxaEntregaComSucesso() throws Exception {
        ...
        mockMvc.perform(get("/api/restaurantes/1/taxa-entrega/01000-000"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.success").value(true))
    ...
    void deveRetornarErroAoCalcularTaxaEntregaParaRestauranteInexistente() throws Exception {
        ...
        mockMvc.perform(get("/api/restaurantes/999/taxa-entrega/01000-000"))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Restaurante não encontrado ou indisponível: Restaurante não encontrado"));
    ...
    void deveBuscarRestaurantesProximosAoCep() throws Exception {
        List<RestauranteResponse> restaurantes = Arrays.asList(
            new RestauranteResponse(1L, "Rest Próximo", "Italiana", "Rua 1", BigDecimal.valueOf(5.0), "11999999999", "proximo@email.com", 30, BigDecimal.valueOf(4.5), true)
        );

        Page<RestauranteResponse> page = new PageImpl<>(restaurantes, PageRequest.of(0, 20), 1);
        when(restauranteService.buscarProximos(eq("01000-000"), any())).thenReturn(page);

        mockMvc.perform(get("/api/restaurantes/proximos/01000-000")
                .param("page", "0")
                .param("size", "20"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void deveListarPedidosDoRestaurante() throws Exception {
        List<PedidoResponse> pedidos = Arrays.asList(
            new PedidoResponse(1L, null, null, null, BigDecimal.valueOf(25.00), null, StatusPedido.CRIADO,
                LocalDateTime.now(), Arrays.asList(new ItemPedidoResponse(1L, "Produto 1", 1, BigDecimal.valueOf(25.00))))
        );
        ...
        mockMvc.perform(get("/api/restaurantes/1/pedidos")
                .param("page", "0")
                .param("size", "20"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void deveListarProdutosDoRestaurante() throws Exception {
        ...
        List<ProdutoResponse> produtos = Arrays.asList(
            new ProdutoResponse(1L, "Pizza Margherita", "Pizza", "Deliciosa pizza", BigDecimal.valueOf(35.00),
                new RestauranteResumoResponse(1L, "Restaurante Teste", "Italiana", BigDecimal.valueOf(5.0), 30, BigDecimal.valueOf(4.5), true), true, 10)
        );
    ...
    void deveRetornarErroAoListarProdutosDeRestauranteInexistente() throws Exception {
        ...
        mockMvc.perform(get("/api/restaurantes/999/produtos"))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.success").value(false))
                .andExpect(jsonPath("$.message").value("Restaurante não encontrado"));
