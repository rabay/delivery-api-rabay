  ...
  class ConstructorGettersTests {
    ...
    void deveCriarErrorResponseComTodosOsCampos() {
      ...
      ErrorResponse errorResponse =
          new ErrorResponse(error, message, status, timestamp, path, details);
    ...
    void deveCriarErrorResponseSemDetalhes() {
      ...
      // When
      ErrorResponse errorResponse =
          new ErrorResponse(error, message, status, timestamp, path, null);
  ...
  class CommonErrorScenariosTests {
    ...
    void deveCriarErroRequisicaoInvalida() {
      ...
      Map<String, Object> details =
          Map.of("field", "email", "message", "Email deve ter formato válido");

      // When
      ErrorResponse errorResponse =
          new ErrorResponse(
              "BadRequest", "Requisição inválida", 400, timestamp, "/api/clientes", details);
    ...
    void deveCriarErroRecursoNaoEncontrado() {
      ...
      ErrorResponse errorResponse =
          new ErrorResponse(
              "NotFound", "Cliente não encontrado", 404, timestamp, "/api/clientes/999", null);
    ...
    void deveCriarErroConflito() {
      ...
      OffsetDateTime timestamp = OffsetDateTime.now();
      Map<String, Object> details =
          Map.of("field", "email", "message", "Email já cadastrado no sistema");

      // When
      ErrorResponse errorResponse =
          new ErrorResponse(
              "Conflict", "Dados conflitantes", 409, timestamp, "/api/clientes", details);
      ...
      assertThat(errorResponse.details())
          .containsEntry("message", "Email já cadastrado no sistema");
    }
    ...
    void deveCriarErroInternoServidor() {
      ...
      OffsetDateTime timestamp = OffsetDateTime.now();
      Map<String, Object> details =
          Map.of("traceId", "abc-123-def", "operation", "processar_pedido");
    ...
    void deveCriarErroAutenticacao() {
      ...
      ErrorResponse errorResponse =
          new ErrorResponse(
              "Unauthorized", "Credenciais inválidas", 401, timestamp, "/api/auth/login", null);
    ...
    void deveCriarErroAutorizacao() {
      // Given
      OffsetDateTime timestamp = OffsetDateTime.now();
      Map<String, Object> details = Map.of("requiredRole", "ADMIN", "userRole", "USER");

      // When
      ErrorResponse errorResponse =
          new ErrorResponse(
              "Forbidden", "Acesso negado", 403, timestamp, "/api/admin/clientes", details);
      ...
      Map<String, Object> details =
          Map.of(
              ...
              Map.of(
                  ...
                  "Nome é obrigatório",
                  "endereco",
                  Map.of("rua", "Rua é obrigatória", "numero", "Número deve ser positivo")),
              "totalErrors",
              3);
