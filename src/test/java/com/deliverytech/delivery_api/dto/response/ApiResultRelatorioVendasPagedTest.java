...
class ApiResultRelatorioVendasPagedTest {
  ...
  static class MockRelatorioVendas
      implements com.deliverytech.delivery_api.projection.RelatorioVendas {
    private final String nomeRestaurante;
    private final BigDecimal totalVendas;
    private final Long quantidadePedidos;

    public MockRelatorioVendas(
        String nomeRestaurante, BigDecimal totalVendas, Long quantidadePedidos) {
  ...
  void deveDefinirEObterDadosRelatorioVendasPaginado() {
    // Given
    MockRelatorioVendas relatorio1 =
        new MockRelatorioVendas("Pizzaria do João", new BigDecimal("1250.50"), 25L);
    MockRelatorioVendas relatorio2 =
        new MockRelatorioVendas("Hamburgueria Express", new BigDecimal("890.75"), 18L);

    List<com.deliverytech.delivery_api.projection.RelatorioVendas> items =
        List.of(relatorio1, relatorio2);
    Map<String, String> links =
        Map.of("first", "/api/relatorios/vendas?page=0", "last", "/api/relatorios/vendas?page=2");

    PagedResponse<com.deliverytech.delivery_api.projection.RelatorioVendas> pagedData =
        new PagedResponse<>(items, 43, 0, 10, 5, links, "Relatório de vendas", true);
  ...
  void deveCriarApiResultRelatorioVendasPagedComRelatorioPaginaEspecifica() {
    // Given
    MockRelatorioVendas relatorio =
        new MockRelatorioVendas("Restaurante Premium", new BigDecimal("5000.00"), 100L);
    ...
    Map<String, String> links =
        Map.of(
            ...
            "prev",
            "/api/relatorios/vendas?page=1",
            "self",
            "/api/relatorios/vendas?page=2",
            "next",
            "/api/relatorios/vendas?page=3",
            "last",
            "/api/relatorios/vendas?page=4");

    PagedResponse<com.deliverytech.delivery_api.projection.RelatorioVendas> pagedData =
        new PagedResponse<>(items, 500, 2, 10, 50, links, "Página 3 de 50", true);
  ...
  void deveCriarApiResultRelatorioVendasPagedComValoresZero() {
    // Given
    MockRelatorioVendas relatorio =
        new MockRelatorioVendas("Novo Restaurante", BigDecimal.ZERO, 0L);

    List<com.deliverytech.delivery_api.projection.RelatorioVendas> items = List.of(relatorio);
    PagedResponse<com.deliverytech.delivery_api.projection.RelatorioVendas> pagedData =
        new PagedResponse<>(items, 1, 0, 10, 1, Map.of(), "Restaurante sem vendas", true);
