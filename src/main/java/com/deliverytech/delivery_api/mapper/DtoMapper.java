...
public class DtoMapper {
  ...
  public <T> List<T> toDtoList(List<?> source, Class<T> targetClass) {
    return source.stream().map(item -> toDto(item, targetClass)).collect(Collectors.toList());
  }

  public <T> List<T> toEntityList(List<?> source, Class<T> targetClass) {
    return source.stream().map(item -> toEntity(item, targetClass)).collect(Collectors.toList());
  }
}
